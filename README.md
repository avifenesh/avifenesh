...
**[tiyuvta](https://tiyuvta.ai)** is my research lab, and it sells one thing: self-deploying open-source frontier models on hardware you can afford, cheaper and faster than the stock recipe. The work is choosing the right models for your load (chat, embedding, rerank, specialists: what fits and what doesn't), fine-tuning on your workload, reworking the model's architecture and the serving engine so it fits the cards you can buy, and setting it up in your cloud when you'd rather not run hardware. Days on site when it helps. The researcher who builds the deployment is the one who keeps supporting it. Proof you can try before we talk: DeepSeek-V4.1-Flash, served by us, on an OpenAI-compatible endpoint: [put a few dollars in and run your own workload against it](https://tiyuvta.ai/services/try/). Public latency and uptime at [status.tiyuvta.ai](https://status.tiyuvta.ai). Client examples: **[tiyuvta-use-cases](https://github.com/avifenesh/tiyuvta-use-cases)** · **[llm-tiyuvta](https://github.com/avifenesh/llm-tiyuvta)**.

## ML research

- **[memra](https://github.com/avifenesh/memra)**: from-scratch Rust + CUDA research engine for RTX PRO 6000 Blackwell and RTX 5090, the instrument behind the studies below. Safetensors is the tuned path, GGUF stays supported, and a mechanism that wins on one card and loses on the other becomes a per-device default rather than a compromise. On crates.io with prebuilt binaries.
